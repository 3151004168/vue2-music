  ...
  <div class="mv">
    <div class="mv-left">
      <h2 @click="goBack()" style="cursor: pointer">
        <i class="iconfont icon-back"></i> MV详情
      </h2>
      <div class="mv-info">
        <video :src="mvUrl" controls></video>
      </div>
      <div class="avatar">
        <a-avatar :src="artists.img1v1Url" :size="64">
      ...
      <div class="brief">
        ...
        <p style="color: #999">
          发布：{{ mvInfo.publishTime }} &nbsp;&nbsp;&nbsp;&nbsp;播放：{{
            10000 > mvInfo.playCount
              ? mvInfo.playCount
              : Math.round(mvInfo.playCount / 10000) + "万"
          }}
        </p>
      </div>
      <div v-if="isUnfold" class="desc">{{ mvInfo.desc }}</div>
      <div class="btn">
        ...
        <a-button style="border-radius: 50px; margin-right: 10px">
          <i class="iconfont icon-shoucanggedan"></i>收藏{{
            mvInfo.subCount ? "(" + mvInfo.subCount + ")" : ""
          }}</a-button
        >
        <a-button style="border-radius: 50px; margin-right: 10px">
    ...
    <div class="recom">
      ...
      <div class="bottom">
        ...
        >
          ...
          <div class="text">
            {{ item.name }}
            <p>{{ item.artistName ? "by" + " " + item.artistName : "" }}</p>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async getMvInfo() {
      const { data } = await axios("/mv/detail", {
        params: { mvid: this.id },
      });
    ...
    async getCommentInfo() {
      const data = await axios("/mv/detail/info", {
        params: {
          mvid: this.id,
        },
      });
    ...
    async getSimilarMv() {
      const { data } = await axios("/simi/mv", {
        params: {
          mvid: this.id,
        },
      });
      ...
      for (let i = 0; i < this.similarMv.length; i++) {
        this.similarMv[i].duration = fomatMinute(this.similarMv[i].duration);
      }
      console.log(this.id);
    },
    goBack() {
